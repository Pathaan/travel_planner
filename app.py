...
st.set_page_config(page_title="🌍 Desi Adventure Agency", layout="wide")
st.markdown(
    """
    <style>
        .title {
...
# Title and subtitle
st.markdown('<h1 class="title">✈️ Desi Adventure Agency</h1>', unsafe_allow_html=True)
...
hotel_rating = st.sidebar.selectbox("🛏️ Preferred Hotel Rating:", ["Any", "3⭐", "4⭐", "5⭐"])
...
currency_converter = st.sidebar.checkbox("💰 Currency Exchange")
...
researcher = Agent(
    ...
    tools=[SerpApiTools(api_key=os.getenv("SERPAPI_KEY"))],
...
if st.button("🌟 Create Your Travel Plan"):
    with st.spinner("🛫 Searching top flight options..."):
    ...
    # Display Results
    st.subheader("🏝️ Desi type Options")
    if cheapest_flights:
    ...
    st.subheader("🛏️ Hotels & Restaurants")
